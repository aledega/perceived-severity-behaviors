...
def metrics_plot_both(var_dict, sigma_var_dict, deaths_dict, peak_high_dict, peak_date_dict, grow, function_types, savefig = None):
    ...
    function_types_labels1 = ['Growth', 'Central Growth', 'Start End Growth', 'Start Growth', 'End Growth']
    function_types_labels2 = ['Decrease', 'Central Decrease', 'Start End Decrease', 'Start Decrease', 'End Decrease']
    ...
    for i, metric in zip(range(3), [deaths_dict, peak_high_dict, peak_date_dict]):
        for j, fun_type in zip(range(5), function_types):
            ...
            if i != 2:
                cntrf = axes[i,j].tricontourf(triang, metric[fun_type], levels = 14, cmap = cmap_list[i], norm = colors.Normalize(min_list[i], max_list[i]))##, cmap=cm.coolwarm)
            else:
                cntrf = axes[i,j].tricontourf(triang, metric[fun_type], levels = 14, cmap = cmap_list[i], norm = colors.Normalize(25,75), extend="both")
            if i == 0:
                if grow:
                    axes[i, j].set_title(function_types_labels1[j], fontsize = 30, pad = 15)
                else:
                    axes[i, j].set_title(function_types_labels2[j], fontsize = 30, pad = 15)
            if j == 0:
                if grow:
        ...
        if i == 0:
            ...
            cbar = fig.colorbar(cm.ScalarMappable(norm=colors.Normalize(25,75), cmap=cmap_list[i]), ax=axes[i,:])
            cbar.ax.tick_params(labelsize=20)
            cbar.set_ticks(np.linspace(25,75, 6))
            cbar.set_ticklabels([f'{tick:.0f}' for tick in np.linspace(25,75, 6)])
...
def epi_curves_both(country_dict, i0, icu0, r0, d0, initial_date, t_max, tV0, beta, epsilon, omega, mu, delta, chi, f, IFR, IICUR, PICUD, ICU_max, r_v, type_v, VE_S, VE_Symp, VE_D, alpha, gamma, a0, b0, t_step, maxsigma, grow, fun_type, savefig = None):
    ...
    for i, comp in zip(range(len(comp_to_plot)), comp_to_plot):
        for j, sigma_var in zip(range(len(sigma_var_list)), sigma_var_list):
            if grow:
                ...
                y, dates = integrate_model(country_dict, i0, icu0, r0, d0, initial_date, t_max, tV0, beta, epsilon, omega, mu, delta, chi, f, IFR, IICUR, PICUD, ICU_max, r_v, type_v, VE_S, VE_Symp, VE_D, [alpha]*5, [gamma]*5, var_vec, b0, t_step)
            else:
                var_vec = array_persev_computation(country_dict['Nij'], b0, sigma_var, fun_type, grow=False)
                y, dates = integrate_model(country_dict, i0, icu0, r0, d0, initial_date, t_max, tV0, beta, epsilon, omega, mu, delta, chi, f, IFR, IICUR, PICUD, ICU_max, r_v, type_v, VE_S, VE_Symp, VE_D, [alpha]*5, [gamma]*5, a0, var_vec, t_step)
    ...
    fig.subplots_adjust(top=0.9, left=0.1, right=0.9, bottom=0.3)
...
def epi_comparison_both(country_dict, i0, icu0, r0, d0, initial_date, t_max, tV0, beta, epsilon, omega, mu, delta, chi, f, IFR, IICUR, PICUD, ICU_max, r_v, type_v, VE_S, VE_Symp, VE_D, alpha, gamma, a0, b0, t_step, sigma_list, comp, fun_type, savefig = None):

    fig, axes = plt.subplots(len(sigma_list),len(sigma_list), figsize=(len(sigma_list)*5, len(sigma_list)*6), sharey='all', sharex = 'all')

    label_dict = {0: 'Per. Sev. 1', 1: 'Per. Sev. 2', 2: 'Per. Sev. 3', 3: 'Per. Sev. 4', 4: 'Per. Sev. 5'}

    for i, sigma_b0 in zip(range(len(sigma_list)), sigma_list):

        b0_vec = array_persev_computation(country_dict['Nij'], b0, sigma_b0, fun_type, grow=False)

        for j, sigma_a0 in zip(range(len(sigma_list)), sigma_list):
            
            a0_vec = array_persev_computation(country_dict['Nij'], a0, sigma_a0, fun_type, grow=True)
            y, dates = integrate_model(country_dict, i0, icu0, r0, d0, initial_date, t_max, tV0, beta, epsilon, omega, mu, delta, chi, f, IFR, IICUR, PICUD, ICU_max, r_v, type_v, VE_S, VE_Symp, VE_D, [alpha]*5, [gamma]*5, a0_vec, b0_vec, t_step)
            axes[i,j].grid(True, color = '0.8', which='major')
            axes[i,j].grid(True, color = '0.8', alpha = 0.5, which='minor', linestyle='--')
            axes[i,j].xaxis.set_major_locator(MultipleLocator(100))
            axes[i,j].xaxis.set_minor_locator(MultipleLocator(25))
    ...
    cols = [r'$\sigma²_{a_0}$ = ' + str(round(i,3)) for i in sigma_list]
    for ax, col in zip(axes[0], cols):
        ax.set_title(col, fontsize = 24, pad = 30)

    pad = 30 # in points  
    rows = [r'$\sigma_{b_0}$ = ' + str(round(i,3)) for i in sigma_list]
    for ax, row in zip(axes[:,0], rows):
        ax.annotate(row, xy=(0, 0.5), xytext=(-ax.yaxis.labelpad-pad,0),
                    xycoords=ax.yaxis.label, textcoords='offset points',
